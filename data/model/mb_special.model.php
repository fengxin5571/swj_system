...
class mb_specialModel extends Model{
    ...
    private function _initMbSpecialItemData($item_data, $item_type) {
	    ...
	    // 33hao.com v3-10
            if($item_type == 'goods'||$item_type == 'goods1'||$item_type == 'goods2'||$item_type=="goods_self") {
                $item_data = $this->_initMbSpecialItemGoodsData($item_data, $item_type);
                if($item_type == 'goods1'){//为限时折扣时
                    $xianshi_array = Model('p_xianshi');
                    $condition['state'] = 1;
                    $xianshi = $xianshi_array->getXianshiList($condition,1);
                    if(!empty($xianshi)){
                        $item_data['end_time']=$xianshi[0]['end_time'];
                    }
                }
            }
        } else {
    ...
    private function _initMbSpecialItemGoodsData($item_data, $item_type) {
        ...
        if(!empty($item_data['item'])) {
            ...
            $goods_list = $model_goods->getGoodsList($condition, 'goods_id,goods_name,goods_promotion_price,goods_price,goods_image,goods_storage');
    ...
    public function getMbSpecialModuleList() {
        ...
        $module_list['goods_self'] = array('name' => 'goods' , 'desc' => '商品版块');
	// 33hao.com v3-10
    if(!$_GET['special_id']) {
